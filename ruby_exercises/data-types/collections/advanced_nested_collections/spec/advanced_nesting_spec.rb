...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 3' do
    ...
    risotto_price = stores[:olive_garden][:dishes].select{ |dish| 
      dish[:name].downcase == "risotto"
    }[0][:price]
  ...
  it 'test 4' do
    ...
    big_mac_ingredients = stores[:macdonalds][:dishes].select{ |dish| 
      dish[:name] == "Big Mac"
  }[0][:ingredients]
  ...
  it 'test 7' do
    ...
    # all restaurants
    employee_names = stores.keys.map{ |store|
        stores[store][:employees]
    }.flatten
  ...
  it 'test 8' do
    ...
    ingredients = stores.keys.map{ |store|
